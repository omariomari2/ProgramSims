...
class DataProcessor:
        ...
        try:
            with open(transaction_file, "r") as file:
                ...
                for row in reader:
                    ...
                    # Update customer totals using dictionary lookup
                    # Optimization: Direct access is efficient (O(1))
                    if row["customer_id"] in self.customers:
                        customer = self.customers[row["customer_id"]]
                        customer["total_spent"] += float(row["amount"])
                        customer["transaction_count"] += 1
                    else:
                        logger.warning(
                            f"Transaction for unknown customer: {row['customer_id']}"
                        )
    ...
    def calculate_customer_metrics(self) -> Dict[str, Any]:
        ...
        # Optimization: Use nlargest instead of full sort for better performance on large lists if needed, 
        # but sort is fast enough for typical data sizes here.
        ...
        try:
            ...
            if format == "csv":
                with open(output_file, "w", newline="") as file:
                    # Fix: Explicitly define fieldnames to ensure consistency and avoid runtime errors
                    # if the first customer record lacks keys or if the dictionary order varies.
                    # We use a standard set of keys + any extra keys found in the first record as a fallback.
                    sample_keys = list(next(iter(self.customers.values())).keys())
                    fieldnames = ["customer_id"] + sample_keys
                ...
                with open(output_file, "w") as file:
                   # Fix: Ensure self.customers is treated as a dict. 
                   # If self.customers was somehow corrupted, this might fail, but regular dicts work fine.
                   if isinstance(self.customers, dict):
                        json.dump(self.customers, file, indent=2)
                   else:
                        raise AttributeError(f"Expected dict, got {type(self.customers)}")
            else:
